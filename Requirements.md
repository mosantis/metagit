...
I want to build a command line tool (metagit) in rust to enhance git functionality when dealing with multiple repositories without having to deal with git submodules. The user would provide a .mgitconfig.yaml file in the current directory with the following structure:
...
- The command "mgit init" can do an in-depth inspection of the repositories in the current directory and generate a .mgitconfig.yaml file if it doesn't exist.
